<template>
  <div class="blog-post-view">
    ...
    <div v-if="blogStore.loading" class="animate-pulse" role="status" aria-label="Cargando artículo">
      <div class="w-full h-[clamp(18rem,50vw,30rem)] bg-gray-200"></div>
      <main class="w-full max-w-7xl mx-auto px-4 md:px-12 mt-8">
        <div class="h-4 w-32 bg-gray-200 rounded mb-8"></div>
        <div class="flex flex-col md:flex-row items-start gap-8 mb-16">
          <article class="flex-1 w-full">
            <div class="h-8 w-3/4 bg-gray-200 rounded mb-6"></div>
            <div class="h-4 w-full bg-gray-200 rounded mb-2"></div>
            <div class="h-4 w-full bg-gray-200 rounded mb-2"></div>
            <div class="h-4 w-5/6 bg-gray-200 rounded mb-6"></div>
            <div class="h-4 w-full bg-gray-200 rounded mb-2"></div>
            <div class="h-4 w-4/5 bg-gray-200 rounded mb-2"></div>
          </article>
          <aside class="w-full md:w-[280px] shrink-0 h-[250px] bg-gray-200 rounded-xl"></aside>
        </div>
      </main>
      <span class="sr-only">Cargando contenido del artículo...</span>
    </div>

    <template v-else-if="post">
      <!-- Hero del artículo -->
      <section class="relative w-full h-[clamp(18rem,50vw,30rem)] overflow-hidden" aria-label="Cabecera del artículo">
        <img :src="post.image" :alt="post.imageAlt || `Imagen de cabecera para ${post.title}`" class="w-full h-full object-cover brightness-50 saturate-[1.3]" fetchpriority="high" />
        <div class="absolute inset-0 flex items-end bg-gradient-to-t from-black/70 to-transparent p-10 md:px-12">
          <div class="max-w-4xl slide-left">
            <span v-if="post.category" class="inline-block bg-atomic text-white font-sans text-sm font-bold px-4 py-1.5 rounded-full mb-3">{{ post.category }}</span>
            ...
            <div class="flex flex-wrap gap-5 font-sans text-sm text-white/80 mt-2">
              ...
              <span class="flex items-center gap-1.5"><i class="bi bi-clock" aria-hidden="true"></i> {{ post.readTime || 5 }} min de lectura</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <main class="w-full max-w-7xl mx-auto px-4 md:px-12">
        ...
        <div class="flex flex-col md:flex-row items-start gap-8 mb-16">
          <!-- Contenido principal -->
          <article class="flex-1 min-w-0" aria-label="Contenido del artículo">
            <div class="article-body">
              <PortableText :value="post.content" />
            </div>

            <!-- Tags -->
            <div v-if="post.tags && post.tags.length" class="flex flex-wrap gap-2 my-8" role="list" aria-label="Etiquetas del artículo">
        ...
        <!-- También han leído -->
        <section v-if="relatedPosts.length" class="my-16" aria-labelledby="related-heading">
            ...
            >
              <RouterLink :to="`/blog/${related.slug}`" class="block group" :aria-label="`Leer artículo: ${related.title}`">
                <div class="relative h-[200px] overflow-hidden">
                  <img :src="related.image" :alt="related.imageAlt || related.title" loading="lazy" class="w-full h-full object-cover transition-transform duration-400 group-hover:scale-[1.06]" />
                  <span v-if="related.category" class="absolute top-3 left-3 bg-atomic text-white font-sans text-[0.7rem] font-bold px-3 py-1 rounded-full">{{ related.category }}</span>
                </div>
              </RouterLink>
              <div class="p-5 flex flex-col gap-2 flex-1">
                <time :datetime="related.publishedAt" class="font-sans text-[0.8rem] text-secondary flex items-center gap-1.5">
                  <i class="bi bi-calendar3" aria-hidden="true"></i> {{ formatDate(related.publishedAt) }}
                </time>
    ...
    <!-- 404 post not found -->
    <div v-else-if="!blogStore.loading && !post" class="flex flex-col items-center justify-center text-center gap-6 min-h-[60vh] p-8">
      <h1 class="font-serif text-navigation text-3xl">Artículo no encontrado</h1>
      <p class="font-sans text-primary">Lo sentimos, el artículo que buscas no existe o hubo un error al cargar.</p>
...
const updateSEO = (title, excerpt, image) => {
...
const loadData = async () => {
  ...
  if (fetchedPost) {
    ...
    updateSEO(fetchedPost.title, fetchedPost.excerpt, fetchedPost.image)
    
    // Fetch related posts after main post is loaded
    relatedPosts.value = await blogStore.fetchRelatedPosts(slug)
  } else {
    post.value = null
  }
}
...
// React to route changes if user clicks a related post
watch(() => route.params.slug, (newSlug) => {
  if (newSlug) {
    post.value = null // reset to show loading/skeleton or just load new data
    loadData()
...
<style scoped>
/* Article body – styles for PortableText content */
...
.article-body :deep(strong) { color: var(--color-navigation); font-weight: bold; }
.article-body :deep(em)     { color: var(--color-atomic); font-style: italic; }
.article-body :deep(ul)     { list-style-type: disc; margin-left: 2rem; margin-bottom: 1.2rem; }
.article-body :deep(ol)     { list-style-type: decimal; margin-left: 2rem; margin-bottom: 1.2rem; }
.article-body :deep(li)     { margin-bottom: 0.5rem; color: var(--color-primary); }
.article-body :deep(blockquote) { 
  border-left: 4px solid var(--color-atomic); 
  padding-left: 1rem; 
  margin: 1.5rem 0; 
  font-style: italic; 
